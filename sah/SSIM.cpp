...
{
    CV_Assert(m.dims <= 2);
    cvInitImageHeader(&img, cvSize(m.cols, m.rows), cvIplDepth(m.flags), m.channels());
...
cv::Mat ssim(cv::Mat &input1, cv::Mat &input2) {
    ...
    // printf("Input 1 cols: %d, rows: %d\n", input1.cols, input1.rows);
    // printf("Input 1 depth: %d, channels: %d\n", input1.depth(), input1.channels());

    IplImage img1; FromMat(img1, input1);
    IplImage img2; FromMat(img2, input2);
    // printf("Image copied to IplImage\n");
